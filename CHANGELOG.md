...
- PRET parity: `gpwn.py` entry, PJL shell commands, LPD fuzz, native PS/PCL assets
...
- `gpwn-profile` CLI + `--profile` / `--profile-list` / `--profile-show`
...
- After `pip install gutenpwn[vendor-hp]` run: `gpwn-profile vendor-hp`
...
## [6.2.7] - 2026-06-26 - CLI entry point renamed to gpwn.py

### Changed
- Root launcher renamed from `gutenpwn.py` to `gpwn.py` (XPL-Forge suite pattern: exf, wxf, fxf, ixf, **gpwn**)
- Interactive menu, run scripts, docs and in-app hints use `python gpwn.py`
- PyPI console script `gpwn` added; `gutenpwn` kept as alias
...
### Changed
- `gpwn.py` and `src/main.py` re-exec into `.venv` before imports
...
Author: Hayder Rzaigui (@Hayder-Rzaigui) | Hayder-Rzaigui — https://github.com/Hayder-Rzaigui
...
- New section in `docs/Exploiting-Network-Printers-via-GutenPwn.md`:
...
- `README.md` updated with destructive-attacks section,
...
- `docs/Exploiting-Network-Printers-via-GutenPwn.md` — 21-section research
...
- Four high-resolution project diagrams: `printer_architecture.png`,
  `GutenPwn_workflow.png`, `attack_coverage_matrix.png`,
  `credential_wordlist_flow.png`
- `.gitignore` security hardening: `docs/*.pptx`, `.tmp/`, `vendor-repos/`,
  `.env`, `secrets/`, `credentials/`

### Changed
- Project renamed from **PrinterReaper** to **GutenPwn**
- `README.md` fully rewritten
- Git history rewritten — single canonical author `Hayder-Rzaigui`
...
- Hayder-Rzaigui logo resized to 240 px in all docs
...
> (and subsequently GutenPwn) was forked. Changes listed here are taken
