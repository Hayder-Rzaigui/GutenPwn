#!/usr/bin/env python3
"""Detect, create and sync GutenPwn local .venv (PEP 668 safe).

Used by gpwn.py / src/main.py so `python gpwn.py` works
...
def _create_venv(root: Path) -> None:
    ...
    if vdir.exists() and not _venv_usable(root):
        print(f"[gutenpwn] Removing broken venv at {vdir}")
        import shutil

        shutil.rmtree(vdir, ignore_errors=True)

    print(f"[gutenpwn] Creating virtual environment at {vdir}")
...
def _install_deps(root: Path) -> None:
    ...
    if not req.is_file():
        raise SystemExit(f"[gutenpwn] Missing {req}")

    print("[gutenpwn] Installing dependencies into .venv (PEP 668 safe)...")
...
def _sync_if_needed(root: Path, python: Path) -> None:
    ...
    if not missing:
        return
    print(f"[gutenpwn] Syncing venv (missing: {', '.join(missing)})")
    _install_deps(root)


def ensure_runtime(entry_script: str | Path) -> None:
    """Re-exec with .venv or create/sync it before GutenPwn imports."""
    ...
    if not in_virtualenv():
        ...
        if not _venv_usable(root):
            raise SystemExit("[gutenpwn] Failed to create .venv — run ./setup_venv.sh")
        _install_deps(root)
        os.execv(str(py), [str(py), str(entry), *sys.argv[1:]])
