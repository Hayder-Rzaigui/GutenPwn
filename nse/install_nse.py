...
# -*- coding: utf-8 -*-
"""
gutenpwn-nse — NSE Script Installer for GutenPwn
===========================================================
Copies GutenPwn Nmap NSE scripts and the shared gutenpwn.lua library
into the active Nmap script directory so they become available system-wide.

Usage:
    gutenpwn-nse install           # install scripts
    gutenpwn-nse uninstall         # remove scripts
    gutenpwn-nse list              # list installed scripts
    gutenpwn-nse status            # check installation status
    gutenpwn-nse path              # show detected nmap scripts dir
    gutenpwn-nse verify            # verify nmap can load the scripts
...
BANNER = r"""
 ...
 |_|   |_|  |_|_| |_|\__\___|_|  /_/  \_\_|  |_____|  NSE Installer v5.0.0
  ...
  Hayder Rzaigui (@Hayder-Rzaigui) | Hayder-Rzaigui
...
def cmd_install(args: argparse.Namespace) -> int:
    ...
    if scripts_dir is None:
        if args.scripts_dir:
            ...
            print("        Use: gutenpwn-nse install --scripts-dir /path/to/nmap/scripts")
            return 1
    ...
    # Copy shared library (gutenpwn.lua)
    for lib in NSE_LIBS:
...
def cmd_list(_: argparse.Namespace) -> int:
    print(BANNER)
    print(f"GutenPwn NSE Bundle  ({len(NSE_SCRIPTS)} scripts + {len(NSE_LIBS)} libs)\n")
    print("Scripts:")
...
def cmd_status(_: argparse.Namespace) -> int:
    ...
    if missing == 0:
        ...
        print("[NOT INSTALLED]  No files installed yet — run: gutenpwn-nse install")
        return 1
    else:
...
def _print_usage_hint() -> None:
    print()
    print("=" * 68)
    print("  GutenPwn NSE scripts are now available in Nmap!")
    print()
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="gutenpwn-nse",
        description="GutenPwn — Nmap NSE script installer",
    ...
    p_install.add_argument("--nselib-dir", metavar="PATH",
        help="Override Nmap nselib directory (for gutenpwn.lua)")
