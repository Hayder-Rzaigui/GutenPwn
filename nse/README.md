# GutenPwn — Nmap NSE Scripts

> Author: Hayder Rzaigui (@Hayder-Rzaigui) | Hayder-Rzaigui  
> Repo: https://github.com/Hayder-Rzaigui/GutenPwn  
> Docs: https://github.com/Hayder-Rzaigui/GutenPwn/wiki/NSE
...
These scripts complement the full GutenPwn toolkit, providing:
...
```bash
# 1. Install GutenPwn with NSE extras
pip install gutenpwn[nse]

# 2. Deploy scripts to Nmap (requires elevated privileges)
gutenpwn-nse install
...
**Shared library:** `lib/gutenpwn.lua` — vendor patterns, CVE DB, verdict formatting
...
When Nmap suggests a module, run the full exploit with GutenPwn:

```bash
pip install gutenpwn

# Scan all ports
gutenpwn scan --target <IP>

# Run specific module (dry-run first)
gutenpwn run --module xpl/edb-cve-2025-26506 --dry-run --target <IP>
gutenpwn run --module xpl/edb-cve-2025-26506 --target <IP>

# Check without exploiting
gutenpwn check --module xpl/edb-cve-2025-26506 --target <IP>
```
...
nse/
├── __init__.py
├── install_nse.py          ← CLI installer (gutenpwn-nse)
├── README.md
├── scripts/
│   ├── printer-banner.nse
...
└── lib/
    └── gutenpwn.lua      ← Shared Lua library
```

---

> Created by Hayder Rzaigui (@Hayder-Rzaigui) — Hayder-Rzaigui  
> https://github.com/Hayder-Rzaigui
