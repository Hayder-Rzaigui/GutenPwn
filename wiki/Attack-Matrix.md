...
python gpwn.py 192.168.1.100 --attack-matrix --no-dry

# Combined with network mapping (single pass)
python gpwn.py 192.168.1.100 --attack-matrix --network-map

# With debug output (show raw bytes)
python gpwn.py 192.168.1.100 --attack-matrix --debug
```
...
The paper identifies 4 main attack categories (DoS, Protection Bypass, Job Manipulation, Information Disclosure) across PJL, PostScript, PCL, and network protocols. GutenPwn implements all documented techniques plus additional CVEs from 2022-2025.
