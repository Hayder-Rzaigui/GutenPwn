...
python gpwn.py 192.168.1.100 --bruteforce
...
python gpwn.py 192.168.1.100 --bruteforce --bf-vendor oki --bf-mac AA:BB:CC:DD:EE:FF

# Combined with scan (auto-populates vendor + serial from fingerprint)
python gpwn.py 192.168.1.100 --scan --bruteforce
```
...
```bash
python gpwn.py 192.168.1.100 --bruteforce --bf-vendor hp
```

### Custom wordlist (replaces default)

```bash
python gpwn.py 192.168.1.100 --bruteforce --bf-wordlist /path/to/my_creds.txt
```
...
```bash
python gpwn.py 192.168.1.100 --bruteforce --bf-no-variations
```

### Slow down (avoid lockouts)

```bash
python gpwn.py 192.168.1.100 --bruteforce --bf-delay 2.0
```
...
GutenPwn brute-forces all available authentication endpoints in parallel:
