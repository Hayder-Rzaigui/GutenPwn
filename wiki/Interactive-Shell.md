...
Connect and run commands against the printer interactively. GutenPwn supports three printer languages across a unified shell interface.
...
python gpwn.py 192.168.1.100 auto
...
python gpwn.py 192.168.1.100 ps

# PCL (Printer Command Language) — macro filesystem (legacy HP)
python gpwn.py 192.168.1.100 pcl

# Safe mode — verify language supported before connecting
python gpwn.py 192.168.1.100 pjl --safe

# Debug mode — print raw bytes on wire
python gpwn.py 192.168.1.100 pjl --debug

# Non-interactive batch mode — read commands from file
python gpwn.py 192.168.1.100 pjl -i commands.txt

# Log everything sent to printer to file
python gpwn.py 192.168.1.100 pjl -o session.log

# Quiet + batch (scripting-friendly)
python gpwn.py 192.168.1.100 pjl -i cmds.txt -o out.log -q
```
...
```bash
python gpwn.py 192.168.1.100 pjl -i commands.txt -o session.log -q
```
