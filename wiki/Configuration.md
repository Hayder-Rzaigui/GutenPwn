...
GutenPwn reads a `config.json` file for API keys and runtime settings.
...
By default, GutenPwn looks for `config.json` in the project root (same directory as `gpwn.py`).

Use a custom path with:
```bash
python gpwn.py --config /path/to/my_config.json [...]
```
