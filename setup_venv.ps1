# setup_venv.ps1 — Virtual environment for GutenPwn (Windows)
...
Write-Host "=== GutenPwn — Virtual Environment Setup (Windows) ===" -ForegroundColor Cyan
Write-Host ""
...
if (-not (Test-Path $VenvDir)) {
    ...
    & $pythonCmd -m venv $VenvDir --prompt GutenPwn
} else {
...
New-Item -ItemType File -Path (Join-Path $VenvDir ".gutenpwn-venv-ready") -Force | Out-Null

Write-Host ""
Write-Host "=== Environment ready! ===" -ForegroundColor Green
Write-Host "Run:      .\run.ps1 <target> [options]"
Write-Host "Or:       python gpwn.py"
