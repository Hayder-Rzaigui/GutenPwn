#!/usr/bin/env bash
# setup_venv.sh — Virtual environment for GutenPwn
...
echo "=== GutenPwn — Virtual Environment Setup ==="
echo ""
...
if [ ! -d "$VENV_DIR" ] || ! _venv_python; then
    ...
    python3 -m venv "$VENV_DIR" --prompt GutenPwn
...
touch "$VENV_DIR/.gutenpwn-venv-ready"

echo ""
echo "=== Environment ready! ==="
echo ""
echo "Run:      ./run.sh <target> [options]"
echo "Or:       python gpwn.py              # auto-detects .venv"
