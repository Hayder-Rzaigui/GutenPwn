...
# -*- coding: utf-8 -*-
"""
GutenPwn — Configuration Loader
======================================
Loads config.json (JSON) from the project root, a path set via the
--config CLI flag, or the GutenPwn_CONFIG environment variable.
...
Resolution order (highest priority first):
  ...
  2. Environment variable: GutenPwn_CONFIG
...
def _find_config_file(explicit_path: str | None = None) -> Optional[Path]:
    ...
    env_path = os.environ.get('GutenPwn_CONFIG')
    if env_path:
