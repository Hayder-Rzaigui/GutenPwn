...
Persisted: ~/.config/gpwn/profile.json  (override with GPWN_PROFILE env)
"""
...
_CONFIG_DIR = Path(os.environ.get("XDG_CONFIG_HOME") or Path.home() / ".config") / "gpwn"
...
def read_active_profile_name() -> str:
    env = os.environ.get("GPWN_PROFILE", "").strip()
    if env:
        return env
