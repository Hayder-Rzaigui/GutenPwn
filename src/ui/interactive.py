...
# -*- coding: utf-8 -*-
"""
GutenPwn — Interactive Guided CLI
=======================================
...
Launch: python gpwn.py  (no arguments)
        python gpwn.py --interactive
"""
...
def _print_cmd(cmd: List[str]) -> None:
    ...
    print()
    print(f"  {_DIM}GutenPwn v{__version__}{_RST}")
    print(f"  {_DIM}Running command:{_RST}")
    print(f"  {_BLU}$  python gpwn.py {' '.join(cmd)}{_RST}")
    print()
...
def _banner_mini() -> None:
    ...
    print()
    print(f"  {_RED}██████{_RST}{_WHT}╗ {_RED}███████╗{_RST}{_WHT}╗{_RST}  GutenPwn  "
          f"{_DIM}v{__version__} ({__release_date__}){_RST}")
    print(f"  {_RED}██╔══██{_RST}{_WHT}╗{_RED}██╔════╝{_RST}  "
          f"{_DIM}Comprehensive Security Assessment Framework for Network Printers{_RST}")
    print(f"  {_RED}██████╔╝{_RED}█████╗  {_RST}  "
          f"{_DIM}@Hayder-Rzaigui · linkedin.com/in/Hayder-Rzaigui{_RST}")
    print()
    _hr('═', _CYN)
    print()
...
def _show_about() -> None:
    ...
    print(f"  {_BLD}GutenPwn — Comprehensive Security Assessment Framework for Network Printers{_RST}")
    print(f"  {_CYN}{'═'*60}{_RST}")
    lines = [
        ('Author',    'Hayder Rzaigui (@Hayder-Rzaigui)'),
        ('GitHub',    'https://github.com/Hayder-Rzaigui'),
        ('LinkedIn',  'https://linkedin.com/in/Hayder-Rzaigui'),
        ('X',         'https://x.com/Hayder-Rzaigui'),
        ('License',   'MIT'),
...
def run_interactive() -> None:
    ...
    while True:
        ...
        print(f"  {_DIM}[T]  Set/change session target    [Q]  Exit GutenPwn{_RST}")
        print()
