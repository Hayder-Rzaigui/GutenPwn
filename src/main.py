...
# -*- coding: utf-8 -*-
"""
GutenPwn - Comprehensive Security Assessment Framework for Network Printers
Main entry point.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog=CLI_NAME,
        description=f"{APP_NAME} - Comprehensive Security Assessment Framework for Network Printers",
    )
    ...
    parser.add_argument(
        ...
        metavar="NAME",
        default=None,
        help="Custom printer name for --install-printer (default: GutenPwn-<IP>).",
    )
    parser.add_argument(
        "--send-copies",
        metavar="N",
...
def _probe_discovered_targets(hits, limit: int, no_nvd: bool = True) -> None:
    """Quick GPWN validation on IPs returned by online discovery."""
...
def _run_scan(args) -> None:
    ...
    try:
        ...
        print(f"  {_CYN}│{_RST}    {_DIM}python gpwn.py {target} --bruteforce "
        ...
        print(f"  {_CYN}│{_RST}    {_DIM}python gpwn.py {target} --attack-matrix{_RST}")
        print(f"  {_CYN}│{_RST}")
        print(f"  {_CYN}│{_RST}  {_GRN}Network mapping{_RST} (subnet scan, pivot paths):")
        print(f"  {_CYN}│{_RST}    {_DIM}python gpwn.py {target} --network-map{_RST}")
        print(f"  {_CYN}│{_RST}")
        print(f"  {_CYN}│{_RST}  {_YEL}Interactive guided menu:{_RST}")
        print(f"  {_CYN}│{_RST}    {_DIM}python gpwn.py  (no args){_RST}")
        print(f"  {_CYN}└─────────────────────────────────────────────────────────────{_RST}")
    except Exception:
        pass
    ...
    if is_hp_consumer or ('AIRPRINT' in langs and 'PJL' not in langs and not raw_open):
        ...
        output().message(f"  Recommended: python gpwn.py {target} pjl --safe")
    elif ('PS' in langs or 'POSTSCRIPT' in langs) and raw_open:
        output().message(f"  Recommended: python gpwn.py {target} ps --safe")
    elif 'PCL' in langs and raw_open:
        output().message(f"  Recommended: python gpwn.py {target} pcl --safe")
...
def _run_attack_modules(args) -> None:
    ...
    if getattr(args, 'xpl_list', False):
        try:
            ...
            print_exploit_list(
                xpls,
                title=f'GutenPwn Exploit Library{src_label} ({len(xpls)} exploits)'
            )
        except Exception as exc:
...
def intro(quiet: bool) -> None:
    """Print the unified GutenPwn banner."""
...
def main() -> None:
    ...
    if getattr(args, "profile_list", False):
        ...
        print("\n  pip:  pip install gutenpwn[vendor-hp] && gpwn-profile vendor-hp")
        raise SystemExit(0)
        ...
        try:
            ...
            if not dork_params.has_filters() and not getattr(args, 'target', None):
                output().errmsg(
                    ...
                    "Or provide a direct IP target: python gpwn.py <IP> --scan"
                )
                sys.exit(1)
    ...
    if not args.quiet:
        print()
        output().green(f">> Starting {APP_NAME} (Comprehensive Security Assessment Framework for Network Printers)")
        print()
